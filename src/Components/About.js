...
export default function About() {
    ...
    const toggleStyle = () =>{
        if(myStyle.color === "white"){
            ...
            setLevelText("Enable Dark Mode")
        }else{
            setMyStyle({
                color: 'white',
                backgroundColor: 'black'
            })
            setLevelText("Enable Light Mode")
        }
    }
    const [levelText, setLevelText] = useState("Enable Dark Mode");
  return (
    <div className="container py-5">
      ...
      <div className="d-flex justify-content-center mt-2 ">
        <div className="form-check form-switch">
            ...
            <label className="form-check-label" htmlFor="flexSwitchCheckDefault">{levelText}</label>
        </div>
      </div>
    </div>
  );
